            ...
            <form class="mt-6 space-y-4" @submit.prevent="submit">
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select id="email_provider_slug" v-model="form.slug" class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :disabled="mode === 'edit'" required>
                    ...
                    <div>
                        ...
                        <TextInput id="email_provider_name" v-model="form.name" class="mt-1 block w-full" required />
                ...
                <div v-if="needsApiKey">
                    ...
                    <TextInput id="email_provider_api_key" v-model="form.api_key" type="password" class="mt-1 block w-full" :required="mode === 'create'" />
                    <InputError class="mt-1" :message="form.errors.api_key" />
                </div>

                <div>
                    <InputLabel for="email_provider_api_base_url" value="API base URL" />
                    <TextInput id="email_provider_api_base_url" v-model="form.api_base_url" class="mt-1 block w-full" />
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <TextInput id="from_email" v-model="form.metadata.default_from_email" type="email" class="mt-1 block w-full" />
                    </div>
                    <div>
                        <InputLabel for="from_name" value="Default from name" />
                        <TextInput id="from_name" v-model="form.metadata.default_from_name" class="mt-1 block w-full" />
                    </div>
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    <div>
                        ...
                        <TextInput id="priority" v-model="form.priority" type="number" min="1" class="mt-1 block w-full" required />
                    </div>
                    <div>
                        <InputLabel for="timeout" value="Timeout (s)" />
                        <TextInput id="timeout" v-model="form.timeout_seconds" type="number" min="5" class="mt-1 block w-full" required />
                    </div>
                    <div>
                        <InputLabel for="retries" value="Retries" />
                        <TextInput id="retries" v-model="form.retry_count" type="number" min="0" class="mt-1 block w-full" required />
                    </div>
                </div>

                <div>
                    <InputLabel for="status" value="Status" />
                    <select id="status" v-model="form.status" class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
